...
class SSHManager:
    ...
    def create_ssh_client(self, hostname, portname, username, password=None):
...
if __name__ == '__main__':
    ...
    ssh_manager = SSHManager()
    ssh_manager.create_ssh_client(IP, PORT_SSH, 'pi')
    ...
    usr_input = input('do you proceed? ')
    if usr_input != 'y':
        print('=============================')
        print(VERSION_INFO, 'did not released')
        print('=============================')
        import sys
        sys.exit()

    ssh_manager.send_file(
        './version.txt', '/home/pi/change-file-name-api-server/')
    ssh_manager.send_file(
        f'./dist/{VERSION_INFO}.exe', '/home/pi/change-file-name-api-server/release/')
